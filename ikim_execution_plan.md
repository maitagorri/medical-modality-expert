...
**Note on CheXpert Plus label structure:** The main CSV (`df_chexpert_plus_240401`) contains report text sections and metadata but not the 14 binary labels — those are a separate download from the original CheXpert labeler. Download them separately from the CheXpert Labels folder on Redivis (`findings_fixed.json`, `impression_fixed.json`, `report_fixed.json` for report text; the binary labels come from the original CheXpert dataset and need to be joined to the main CSV via `path_to_image`). The EDA script below works with what the CSV actually contains.

**Claude Code prompt for CheXpert Plus EDA:**

> Write a Python script scripts/eda_cxr.py that works with the actual contents of df_chexpert_plus_240401.csv. The script should: load the CSV and print all column names and dtypes so we know exactly what's present; print row counts and missing value counts per column; for any text columns that appear to be report sections (e.g. section_findings, section_impression), print 5 random non-null examples to stdout and report the distribution of text lengths; print the distribution of any demographic or metadata columns present (e.g. sex, age, race if available); display 4 example images from PNG_valid with their path_to_image filename as title and save to /notebooks/figures/chexpert_examples.png; fall back gracefully to text-only output if images are not yet present. Flag explicitly in stdout if the 14 binary label columns are absent and note they need to be joined from a separate file.

**Claude Code prompt for data sampling:**

> Write a script scripts/sample_datasets.py with the following behaviour. For CheXpert Plus: use all 234 images from PNG_valid as the test set (load their paths from the valid split CSV or file list); sample 400 examples from the train split stratified by label availability or report section completeness (whichever columns are present), split into 350 train and 50 validation, saving all three splits as CSV files to /data/processed/chexpert_plus/. Also output a file /data/processed/chexpert_plus/image_list.txt containing the exact path_to_image values needed from PNG_train, so these can be selectively downloaded from Redivis. For PTB-XL: sample 400 examples stratified across the 5 superclass labels (300 train, 50 val, 50 test), saving to /data/processed/ptbxl/. Ensure no patient appears in more than one split using patient_id where available. Print a class balance summary for each split.
